...
export interface PdfOcrOptions {
  ...
  /** When true, refuse OCR unless local traineddata exists — never fetch a CDN (#298, ADR-039). */
  readonly requireLocalTraineddata?: boolean;
  /** DI seam for the local-traineddata pre-flight — defaults to a filesystem probe. */
  readonly hasLocalTraineddata?: LocalTraineddataCheck;
}
...
export type LocalTraineddataCheck = (options: PdfOcrOptions) => Promise<boolean>;
...
// Is eng.traineddata available locally so the worker never reaches a CDN?
// tesseract.js (v7) reads its cache as `${cachePath}/${lang}.traineddata` and a
// local langPath as `${langPath}/${lang}.traineddata[.gz]` — so probe both dirs
// for both the plain and gzipped names. A URL langPath simply will not stat.
export async function hasLocalTraineddata(options: PdfOcrOptions): Promise<boolean> {
  const lang = options.language ?? DEFAULT_OCR_LANGUAGE;
  const dirs = [options.langPath, options.cachePath ?? DEFAULT_CACHE_PATH].filter(
    (dir): dir is string => dir !== undefined
  );
  const candidates = dirs.flatMap((dir) => [
    path.join(dir, `${lang}.traineddata`),
    path.join(dir, `${lang}.traineddata.gz`),
  ]);
  const found = await Promise.all(candidates.map(fileExists));
  return found.some(Boolean);
}

// Strict offline mode (OCR_REQUIRE_LOCAL_TRAINEDDATA): refuse BEFORE spawning a
// worker when no local traineddata exists. tesseract.js spawns the worker thread
// before fetching the model, so a black-hole CDN fetch leaks a worker the bounded
// init timeout cannot terminate (#298, Codex review of PR #311). Refusing
// pre-spawn is the only way to avoid that leak; the bounded init timeout stays
// the safety net for the "data present but init still stalls" case.
async function assertLocalTraineddataIfRequired(options: PdfOcrOptions): Promise<void> {
  if (options.requireLocalTraineddata !== true) return;
  const check = options.hasLocalTraineddata ?? hasLocalTraineddata;
  if (await check(options)) return;
  throw new ParserError(
    'OCR requires local traineddata (set OCR_LANG_PATH or pre-cache eng.traineddata); refusing a CDN fetch'
  );
}
...
async function withRecognizer<T>(
  ...
  await assertLocalTraineddataIfRequired(options);
