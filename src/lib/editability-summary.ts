...
import type { EditabilitySummary } from './jobs.js';
 ...
 * unclassified (structural) nodes are skipped. Shared by the REST onboarding
 * report (`src/api/onboarding-report.ts`) and the MCP `get_onboarding_report`
 * tool (#140), so the two reports cannot drift.
 */
