...
- **The MCP tool surface is contract-bound to the API (ADR-044).** Every user-facing OpenAPI operation maps to an MCP tool or an explicit `MCP_UNEXPOSED` entry (`src/mcp/contract-map.ts`), CI-enforced by `src/mcp/contract.integration.test.ts` (INV-1/2/3). Adding a REST route without a tool (or a reasoned exemption) goes red. Each tool declares a `read`/`write`/`destructive` tier (`src/mcp/capabilities.ts`); `MCP_ALLOWED_TIERS` (default `read,write`) gates which are exposed — destructive/admin actions are off by default (ADR-045).
