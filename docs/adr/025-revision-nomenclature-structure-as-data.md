...
Revision nomenclature — _addendum / bulletin / ASI / proposal request / CCD / milestone
issuance (SD · DD · CD · IFB · IFC)_, and each one's **naming and numbering format** — is
...
to make a revision type _structured_ (a real "short name + number + description + date"
...
- **It breaks the typed contract the system rests on.** TypeScript types, Zod boundary
  validation, `openapi.yaml`, and the module boundaries all target a _known, fixed_ schema.
...
- **Environments diverge.** Dev, staging, prod, and each test run would hold _different
  schemas_ depending on their data, breaking reproducibility, seeding, backups, and schema
...
named, typed contract) _and_ **user-definable at runtime** (defining a type is an API call
...
- **Sequence handling is app-level.** Per-type sequential numbering (`Addendum {number}`)
  cannot be a generated DB column because the number lives in `attributes`. The #209
  foundation stores and validates supplied numbers and keeps `sort_order` as the queryable
  ordering key; automatic per-package allocation is a later query-layer addition.
